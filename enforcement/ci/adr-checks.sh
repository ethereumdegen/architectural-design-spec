...
# ── Terminal UI (tui/0001..0005) — only meaningful in crates that draw a TUI ─────
...
if [ -n "$tui_crates" ]; then
  # tui/0001 (WARN): crossterm as a second direct dependency — use the ratatui re-export.
  ct=$(printf '%s\n' "$tui_crates" | xargs grep -lE '^crossterm[ =]' 2>/dev/null || true)
  [ -n "$ct" ] && { warn "tui/0001" "direct crossterm dependency alongside ratatui — use ratatui::crossterm:"; printf '   %s\n' "$ct"; }

  # tui/0001 (WARN): terminal set up by hand loses ratatui::init's panic-restore hook.
  raw=$(scan 'enable_raw_mode\(|EnterAlternateScreen' || true)
  if [ -n "$raw" ] && ! scan 'ratatui::init\(' >/dev/null; then
    warn "tui/0001" "raw mode entered without ratatui::init() — a panic will leave the terminal broken:"
    printf '   %s\n' "$raw"
  fi

  # tui/0002 (WARN): a TUI with no rendered-screen test.
  scan 'TestBackend' >/dev/null || warn "tui/0002" "crate draws a TUI but no test renders a screen with TestBackend"

  # tui/0003 (WARN): named colours follow the user's theme; the house palette is RGB.
  named=$(scan 'Color::(Red|Green|Blue|Yellow|Cyan|Magenta|White|Black|Gray|Grey)\b' || true)
  [ -n "$named" ] && { warn "tui/0003" "named ratatui colour — use the house Color::Rgb palette:"; printf '   %s\n' "$named"; }

  # tui/0004 (WARN): keys must be advertised — a crate that matches KeyCode::Char needs a key bar.
  if scan "KeyCode::Char\\('" >/dev/null && ! scan 'fn draw_footer|footer|key bar|keybar' >/dev/null; then
    warn "tui/0004" "keys are handled but nothing draws a footer/key bar — every key must be on screen"
  fi

  # tui/0005 (WARN): nothing derived from elapsed time means nothing on screen shows freshness.
  scan 'elapsed\(\)' >/dev/null || warn "tui/0005" "TUI never reads elapsed time — a live view must show that it is live"
fi
